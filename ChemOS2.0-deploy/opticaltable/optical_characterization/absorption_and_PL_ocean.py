...
from pylab.instruments.ocean_spectrometer.ocean_spectrometer import OceanSpectrometer
...
class Abs_PL():
    ...
    def __init__(self, led_power = 80, correction_datafile = None, device = True):
        
        self.device = device

        if self.device:
            self.lamp = ins.DH_mini('ASRL12::INSTR', verbose=False)
            self.spectrometer = OceanSpectrometer('QEP03644')

            self.led_driver = ins.ThorlabsDC4100("ASRL5::INSTR")
            ...
            #If the correction data is obtained with BAM001b-005b, the correction is done to generate the photon spectrum
            ...
            self._initialize_device(led_power)


    def _initialize_device(self, led_power):

        print('***initializing the device***********')
        self.lamp_on()
        self.lamp_shutter_close()

        self.led_on(led_power)
    ...
    def _load_correction_data(self, filename):
        ...
        with open(filename, 'r') as f:
            data = f.readlines()
            for d in data:
                correction_curve.append(float(d.rstrip('\n').split('\t')[-1]))
        return np.asarray(correction_curve)
    ...
    ###led functions#############
    def led_on(self, percentage):

        self.led_365.set_percentage(percentage)
        self.led_365.on()
    ...
    def check_led_state(self):
        status = self.led_365.get_state()

        if status == 'OFF':
            print('causion : uv_led was off. The led might be not stable yet')
    ...
    def measure_uv_power(self, counts, led_power = 80):

        self.check_led_state()
        self.led_on(percentage = led_power)
    ...
    def measure_uv_absorption(self, reference, counts, led_power = 80):

        power = self.measure_uv_power(counts, led_power)

        absorbed_power = reference - power
        absorbed_photon = absorbed_power/self.led_photon_energy  #photons/s
    ...
    def measure_spectrum(self, repeats, integration_time, filter_size = None, dark_correction = True, spectral_correction = False, do_plot = False):
        ...
        spec: np.array = self.spectrometer.measure(repeats=repeats, integration_time=integration_time, dark_correction=False)
    ...
    def measure_spectrum_trace(self, repeats, integration_time, filter_size = None, dark_correction = True, spectral_correction = False, do_plot = False):
        ...
        results = self.spectrometer.measure_trace(repeats = repeats, integration_time = integration_time, dark_correction= False)
        ...
        if spectral_correction:
            ...
            for i, trace in enumerate(results['time_trace']):
                results['time_trace'][i] = trace / self.correction_data
    ...
    def measure_dark_spectrum(self, repeats, integration_time, filter_size = None, do_plot = False):
        ...
        time.sleep(1)
        spec = self.spectrometer.measure(repeats = repeats, integration_time = integration_time, dark_correction=False)
    ...
    def measure_transmission_spectrum(self, repeats: int, integration_time, filter_size = None, dark_correction = True, do_plot = False):
    ...
    def measure_absorption_spectrum(self,  repeats: int, integration_time: float, reference: np.ndarray, filter_size = None, dark_correction = True, do_plot = False):
    ...
    def adjust_PL_exposure(self, initial_exposure, max_exposure, target_intensity, led_power, filter_size = 20, average = 5, max_iter = 5):
        ...
        while saturation and i < max_iter:
            ...
            PL_spectrum = self.measure_PL_spectrum(average, exposure, led_power=led_power, filter_size=filter_size,  \
                dark_correction=False, spectral_correction = False)
    ...
    def measure_PL_spectrum(self, repeats, integration_time, led_power = 80, filter_size = None, dark_correction = True, spectral_correction = True, do_plot = False):
    ...
    def measure_PL_uv(self, repeats, integration_time, led_power = 80, filter_size = None, dark_correction = True, spectral_correction = True, do_plot = False):
        """  
        ...
        def measure_uv(results_uv, flg):
            ...
            while flg['flg'] == True:
                ...
                counts += 1
            results_uv['uv_average'] =  sum(power)/counts
    ...
    def plot_time_trace(self, uv_data, PL_data,  show_plot = True, save_filename = None):
        ...
        fig = plt.figure()
        ax1 = fig.add_subplot(1,1,1, xlabel = 'time/s', ylabel = 'Absorbance(365nm)/PL intensity')
        
        ax1.plot(uv_data['absorbance_time_trace'][0], uv_data['absorbance_time_trace'][1]/uv_data['absorbance_time_trace'][1, 0])
        ax1.plot(PL_data['int_time_trace'][0], PL_data['int_time_trace'][1]/PL_data['int_time_trace'][1, 0])
    ...
    def find_abs_end(self, data, grad_threshold = -0.0005, analysis_range = [300,800], b = 150, c = 40, filter_size = 50):
        """
        ...
        spec = copy.deepcopy(data)[:, l_index: u_index+1]

        fil = np.ones(filter_size)/filter_size
        spec[1] = signal.convolve(spec[1], fil, mode='same')
    
        step = spec[0][1] - spec[0][0]

        b = int(b/step)
        c = int(c/step)

        end_index, end_wavelength = None, None
    
        grad = np.gradient(spec[1])/np.max(spec[1])/step
    
        indices = np.where(grad < grad_threshold)[0]
    
        for i in range(len(indices)-c):
            b = min(b, indices[-i-1])
            if indices[-i-1] - indices[-i-1-c] == c and (spec[1][indices[-i-1]-b:indices[-i-1]] > 0).all():
                end_index = indices[-i-1] + l_index
