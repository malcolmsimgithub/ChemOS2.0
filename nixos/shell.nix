...
let
  ...
  pyro-ppl= python310Packages.buildPythonPackage rec {
    ...
    pyDOE = python310Packages.buildPythonPackage rec {
      pname = "pyDOE";
      version = "0.3.7";
      format = "setuptools";
      doCheck=false;
  ...
  sobol-seq = python310Packages.buildPythonPackage rec {
      pname = "sobol-seq";
      version = "0.2.0";
      format = "wheel";
      doCheck=false;
...
in
  mkShell { buildInputs = with python310Packages; [ pyDOE pip sobol-seq numpy celery sila2 packaging streamlit singleton-decorator python-telegram-bot tkinter pandas typer gpytorch botorch matter-chimera matter-golem olympus atlas psycopg2 rdkit pyunpack patool];

  shellHook = ''
      export OLYMPUS_SCRATCH="/tmp/olympus"
  '';
