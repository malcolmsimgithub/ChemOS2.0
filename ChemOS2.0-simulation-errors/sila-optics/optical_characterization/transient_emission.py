...
class Transient_Emission():
    ...
    def __init__(self, device = True, DB = False):
        ...
        self._load_setting('%s/configs/.config_transient_emission.dat' %filedir) 
        self.device = device
        ...
        if DB:
            ...
            self.db = MDB_client_TE(config_file = '%s/configs/.config_MDB_client.dat' %filedir, admin=False)
    ...
    def detector_off(self, port = 1):
        ...
        port : int
            port number where the detector connected
        """
        # TODO: should this close everything?
        # change to? -> self.pUSB.setport(port, 0)
    ...
    def save_csv(self, filename, results):
        ...
        header, d = [],[] 
        header2 = ['raw_data', '', 'fitting_results']

        if 'fitting_results' in results.keys():
            l.extend(results['fitted_data'].tolist())
            
            for key, val in results['fitting_results'].items():
                header.append(key)
                d.append(val)
    ...
    def measure_TE(self, save_filename = None, **kwargs):
        ...
        if save_filename:
            ...
            self.save_csv(save_filename + '_TE.csv', results)
...
if __name__ == "__main__":
    ...
    folder = 'D:/new/'
    # filename = 'run_optical_23_product_23_4.258min_v3.pkl'
    filenames = glob.glob(folder + '*.pkl')
    for filename in filenames:
        save_filename = '%s_re' %(filename.rstrip('.pkl'))
        data = TE.load_pkl(filename)

        TE.measure_TE_emu(data['TE']['raw_data'], data['TE']['metadata']['excitation_frequency'], save_filename = save_filename, fit_order = 'auto', fit_weight = 0.7, do_plot = True) 
