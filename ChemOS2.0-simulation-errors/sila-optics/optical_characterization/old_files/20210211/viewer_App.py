...
image_file = '%s/mol_image.png' %filepath
...
class Application(tk.Frame):
    ...
    def show_measure_info(self):
        ...
        self.win_mesurement.title('Job info')

        i = 0
        for key, val in d['metadata'].items():
            if key not in ['data_path', 'data_processing', 'solvent']:
                label = tk.Label(self.win_mesurement, text = '%s :' %key, anchor = 'w').place(x = 20 + 220 * i, y = 10) 
                for j, (key1, val1) in enumerate(val.items()):
                    label = tk.Label(self.win_mesurement, text = '%s : %s' %(key1, val1), anchor = 'w').place(x = 40 + 220 * i, y = 35 + 25 *j) 
                i += 1
    ...
    def show_heatmap(self):
        ...
        data_sorted = sorted(self.data_list, key=lambda x:x['PL_max'])        
        data_list = []
        ...
        im, cbar = heatmap(np.asarray(data_list), [d['name'] for d in self.data_list], self.spoke_labels, ax=ax,
