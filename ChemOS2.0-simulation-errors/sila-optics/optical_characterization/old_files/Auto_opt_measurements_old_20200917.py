...
class Optical_measurements():
    ...
    def measure_PL(self, setting):
        ...
        if uv['absorption']/uv['reference'] > 0.003:
            uv['absorbance_maintenance'] = min(1, math.log10(results['uv_end']/uv['reference']) /math.log10(results['uv_start']/uv['reference']))
...
if __name__ == '__main__':
    ...
    Opt = Optical_measurements(TE = False, AbsPL = False)
    ##############
    ...
    # path = 'C:/Users/MatterLab/Dropbox/PythonScript/kazu/data/20200911_test_abs_PL_TE'
    # if os.path.exists(path) == False:
    #    os.mkdir(path)
    # fname = '%s/%s_%s_in_%s_1' %(path, sample_info['concentration(uM)'], sample_info['name'], sample_info['solvent'])

    # Opt.do_measurements(fname, measurements = ['absorption', 'PL'], dilution = 1, water_ratio = 0, **sample_info)
    ...
    Opt.draw_and_dispense('valve', 'vial', 1*ml, draw_velocity=1000, dispense_velocity=1000)
    Opt.draw_and_dispense('vial', 'waste', 1*ml, draw_velocity=1000, dispense_velocity=1000)
    #Opt.draw_and_dispense('valve', 'waste', 1*ml, draw_velocity=1000, dispense_velocity=1000)
    #Opt.draw_and_dispense('valve', 'flow_cell_PL', 0.5*ml, draw_velocity=500, dispense_velocity=500)
    #Opt.draw_and_dispense('valve', 'flow_cell', 0.5*ml, draw_velocity=500, dispense_velocity=500)
    #Opt.draw_and_dispense('valve', 'flow_cell_abs', 0.5*ml, draw_velocity=500, dispense_velocity=500)
    #Opt.draw_and_dispense('valve', 'flow_cell_PL', 0.5*ml, draw_velocity=500, dispense_velocity=500)
    #Opt.draw_and_dispense('valve', 'flow_cell_PL', 0.5*ml, draw_velocity=500, dispense_velocity=500)
    # Opt.draw_and_dispense('valve', 'flow_cell', 0.5*ml, draw_velocity=500, dispense_velocity=500)
