...
class Optical_measurements():
    ...
    def close(self):
        ...
        self.AP.led_off()
        self.AP.lamp_off()
        print(self.AP.lamp.check_status())
        print(self.AP.led_365.get_state())
    ...
    def collect_sample_from_valve(self, sample_volume, collector_num, setting):
        ...
        self.log('washing selector to pump line with solvent')
...
if __name__ == '__main__':
    ...
    path = 'C:/Users/MatterLab/Dropbox/PythonScript/kazu/data/20201102_blank'
    if os.path.exists(path) == False:
       os.mkdir(path)
    fname = '%s/%s_%s_in_%s_via2_3' %(path, sample_info['concentration(uM)'], sample_info['name'], sample_info['solvent'])
    fname2 = '%s/%s_%s_in_%s_via3_1' %(path, sample_info['concentration(uM)'], sample_info['name'], sample_info['solvent'])
    fname3 = '%s/%s_%s_in_%s_via9_1' %(path, sample_info['concentration(uM)'], sample_info['name'], sample_info['solvent'])
    fname4 = '%s/%s_%s_in_%s_via10_1' %(path, sample_info['concentration(uM)'], sample_info['name'], sample_info['solvent'])
    ...
    # #Opt.do_measurements(fname, measurements = ['absorption', 'PL', 'TE'], sample_position = 3, sample_volume = 0.1*ml, dilution = 1, water_ratio = 0, sample_info=sample_info) 
    ...
    Opt.measure_blank(fname, measurements = ['absorption', 'PL'], sample_position = 2, sample_volume = 0.2*ml, sample_info=sample_info) 
    Opt.measure_blank(fname2, measurements = ['absorption', 'PL'], sample_position = 3, sample_volume = 0.2*ml, sample_info=sample_info) 
    Opt.measure_blank(fname3, measurements = ['absorption', 'PL'], sample_position = 9, sample_volume = 0.2*ml, sample_info=sample_info) 
    Opt.measure_blank(fname4, measurements = ['absorption', 'PL'], sample_position = 10, sample_volume = 0.2*ml, sample_info=sample_info) 
    ############
