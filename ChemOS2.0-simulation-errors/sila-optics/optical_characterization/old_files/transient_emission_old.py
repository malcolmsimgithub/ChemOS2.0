...
class Transient_Emission():
    ...
    def _th260_setting(self):
        ...
        self.th260.set_cfd(channel=0,      cfd_level=self.setting['channel_cfd_level'], zero_cross=elf.setting['channel_zero_cross'])
    ...
    def adjust_filter(self, min_rate, max_rate, s_position = 12): 
        ...
        while True:
            ...
            if c_count <  max_rate * s_count:
                if pos == 1 and c_count < min_rate * s_count:
                    raise Exception('Warning : PL intensity did not reach minimum count rate')
                    
                else:
                    self.set_filter(pos-1)
            else: 
                if pos == 12:
                    raise Exception('Warning : Falsed to reduce PL intensity to less than maximum count rate')
                else:
                    self.set_filter(pos+1)
                    break
        self.laser_off()
    ...
    def measure(self, overflow, frequency, timeout=60):
        ...
        self.laser_off()
        x = np.asarray([1e-3 * i * self.res * 2**self.th_binning  for i in range(len(hist))])
        return x, hist
    ...
    def exp_fit(self, x, hist, weight = 1, rise = 50):
        ...
        fit_results ={'amp': fit_vals[0], 'tau' : fit_vals[1], 'x0' : fit_vals[2], 
                                                'bg' : bg, 'max_index' : max_index, 'R2': r_square}
    ...
    def plot_fit(self, data, freq, fname = None, plot = False):
        """  
        ...
        """ 
        fig = plt.figure()
        ax = fig.add_subplot(1,1,1, xlim = [10,0.9*1e9/freq], ylim = [1e0, np.max(data['raw_data'][1])*2], yscale = 'log', xlabel = 'time/ns', ylabel = 'counts')
        ...
        if 'fitting_results' in data.keys():
            ...
            if 'tau1' in data['fitting_results'].keys():
                ax.plot(data['fitted_data'][0], data['fitted_data'][1])
                ax.text(0.7, 0.85, 'tau1 = {:.2f} ns'.format(data['fitting_results']['tau1']), transform=ax.transAxes)
                ax.text(0.7, 0.775, 'amp1 = {:.4f}'.format(data['fitting_results']['amp1']), transform=ax.transAxes)
                ax.text(0.7, 0.7, 'tau2 = {:.2f} ns'.format(data['fitting_results']['tau2']), transform=ax.transAxes)
                ax.text(0.7, 0.625, 'amp2 = {:.4f}'.format(data['fitting_results']['amp2']), transform=ax.transAxes)
                ax.text(0.7, 0.55, 'R2 = {:.4f}'.format(data['fitting_results']['R2']), transform=ax.transAxes)
    ...
    def create_result_dict(self, data, setting):
        ...
        config['experiment_timestamp'] =datetime.now()
    ...
    def measure_TE(self, data, save_filename = None, **kwargs):
        """  
        ...
        # self.adjust_filter(setting['min_rate'], setting['max_rate'], s_position = setting['initial_filter_position']) 
        
        # # set_frequency
        # x, hist = self.measure(100, setting['initial_freq'])
        # fit_res = self.exp_fit(x, hist, weights = setting['fit_weight'])
        ...
        x, hist = data['raw_data'][0], data['raw_data'][1]
        freq = 2E7
        #fitting
        try:
            if setting['fit_order'] == 1:
                fit_res = self.exp_fit(x, hist, weight = setting['fit_weight'])    
                x_fit = x[fit_res['max_index']:]
                hist_fit = [fit_res['amp']*np.exp(-1*(xx-x[fit_res['max_index']]-fit_res['x0'])/fit_res['tau'])+fit_res['bg'] for xx in x_fit]
            
            elif setting['fit_order'] == 2:
                fit_res = self.double_exp_fit(x, hist, weight = setting['fit_weight'])    
                x_fit = x[fit_res['max_index']:]
                hist_fit = [fit_res['amp1']*np.exp(-1*(xx-x[fit_res['max_index']]-fit_res['x0'])/fit_res['tau1'])+\
                    fit_res['amp2']*np.exp(-1*(xx-x[fit_res['max_index']]-fit_res['x0'])/fit_res['tau2'])+fit_res['bg'] for xx in x_fit]

            data = {'raw_data': np.vstack((x, hist)), 'fitted_data': np.vstack((x_fit, hist_fit)), 'fitting_results' : fit_res}

        except:
            print('Fitting error! Only the raw data will be saved')
            data = {'raw_data': np.vstack((x, hist))}
       
        results = self.create_result_dict(data, setting)

        #plot results
        self.plot_fit(data, freq, fname = save_filename, plot = setting['do_plot'])        
        
        #self.close_all()

        #export results
        if save_filename:
            self.save_pkl(save_filename + '.pkl', data)
    ...
    #def measure_TE(self, save_filename = None, **kwargs):
    def measure_TE2(self, data, save_filename = None, **kwargs):
        """  
        Carry out the transient emission measurement based on the setting parameters.
        ...
        if setting['fit_order'] == 1 or setting['fit_order'] == 'both'
            try:
                fit1_res = self.exp_fit(x, hist, weight = setting['fit_weight']) 
                fit1_R2 = fit1_res['R2']   
            except:
                print('Fitting error (1st order)')
        if setting['fit_order'] == 2 or setting['fit_order'] == 'both'
            try:
                fit2_res = self.double_exp_fit(x, hist, weight = setting['fit_weight'])
                fit2_R2 = fit2_res['R2']   
            except:
        ...
        if fit1_R2 == 0 and fit2_R2 == 0:
            ...
            data = {'raw_data': np.vstack((x, hist)), 'fitted_data': np.vstack((x_fit, hist_fit)), 'fitting_results' : fit1_res}
            ...
            data = {'raw_data': np.vstack((x, hist)), 'fitted_data': np.vstack((x_fit, hist_fit)), 'fitting_results' : fit2_res}
