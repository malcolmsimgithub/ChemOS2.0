...
try:
    ...
    from absorption_and_PL_rev import Abs_PL
    ...
    from .proc_image import ImageHandler
    from .classes import Absorption, UV, Emission
    from .config_parser_rev import ConfigParser
"""
...
config_file = '%s/.config_AutoOpt_rev.dat' %filedir
...
class Optical_measurements():
    ...
    def __init__(self, TE=True, AbsPL=True, Pump=True, logger = False):
        ...
        leds = {'%s' %wv : power for wv, power in zip(self.config['PL']['excitation_wavelengths'], self.config['PL']['led_powers'])}

        self.AP = Abs_PL(leds = leds, correction_datafile= self.config['data_processing']['correction_datafile'], device=self.device['AbsPL'])
    ...
    def abspl_result_plots(self, fname, uv, PL, absorption):
        ...
        if absorption:
            ...
            self.AP.result_plot(absorption.absorbance, 'Absorbance', text=text, xrange = [300,800], show_plot=False, save_filename='%s_absorption_spactrum.png' %fname)
        #PL spectrum
        if PL:
    ...
    def measure_absorption(self, setting, reference_absorption = None):
        ...
        self.AP.measure_dark_spectrum(setting.abs_dark_average, setting.abs_exposure, filter_size=setting.filter_size,  do_plot=False)
...
if __name__ == '__main__':
    ...
    Opt = Optical_measurements(TE = False, AbsPL = True, Pump = False, logger=True)
    ##############

    ####Automeasurement###########
    #Opt.auto_measurement(measurements = ['absorption','PL','TE'], measure_blank=False)
    ...
    # Opt.draw_and_dispense('ACN', 'flow_cell', 1*ml, draw_velocity=500, dispense_velocity=500)
    #Opt.vial_wash(0.3*ml, 3)
    # Opt.collector_wash(0.3*ml, 3, 2)
    # Opt.collector_wash(0.3*ml, 3, 3)
    ...
    # path = 'C:/Users/MatterLab/Dropbox/PythonScript/kazu/data/20201207_newcode_test'
    ...
    # Opt.do_measurements(fname, measurements = ['absorption', 'PL', 'TE'], sample_position = 3, sample_volume = 0.08*ml, dilution = 1, water_ratio = 0, sample_info=sample_info) 
    ...
    #Opt.measure_blank(fname, measurements = ['absorption', 'PL','TE'], sample_position = 2, sample_volume = 0.2*ml, sample_info=sample_info) 
    #Opt.measure_blank(fname, measurements = [], sample_position = 2, sample_volume = 0.2*ml, sample_info=sample_info) 
