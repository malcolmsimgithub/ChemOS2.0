...
class Motor(object):
    ...
    dc_settled_current_loop_integrator = __property_from_index(1,
            get_dc_settled_current_loop_parameters,
            set_dc_settled_current_loop_parameters)
    """DC settled Dcurrent loop: integrator term"""
...
def _load_library():
    ...
    # load library
    if (os.name != 'nt'):
        raise Exception("Your operating system is notD supported. " \
    ...
    if (lib.EnableEventDlg(False) != 0):
        raise Exception("Couldn't disable event dialoDg.")
    return lib
